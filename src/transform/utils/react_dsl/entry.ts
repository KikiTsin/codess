...
export default async function (schema, option) {
  // get blocks json
  const blocks: any[] = [];
  const pages: any[] = [];
  ...
  schema.css = schema.css || (option.useTailwind ? '' : defaultGlobalCss);
  ...
  if (option.useTailwind) {
    tailwindStyle(schema);
  }

  // 提取全局样式，类名数组存于 json.classString , 剩余样式覆盖 style
  traverse(schema, (json) => {
    let classnames: string[] = json.classnames || [];
    let tailwindClassnames: string[] = json.tailwindClassnames || [];
  ...
  if (blocks.length > 0) {
    for (const block of blocks) {
      const result = await exportBlock(block, option);
  ...
  if (schema.componentName === 'Page') {
    const result = await exportBlock(schema, option);
    panelDisplay = panelDisplay.concat(result);
  }
  ...
  if (isProject) {
    ...
    // 项目文件
    panelDisplay = panelDisplay.concat(
      await exportCreateApp(schema, { ...option, dependencies })
    );
  }

  // 全局样式
  panelDisplay = panelDisplay.concat(
    await exportGlobalCss(schema, { ...option, folder: isProject ? 'src' : '' })
  );
