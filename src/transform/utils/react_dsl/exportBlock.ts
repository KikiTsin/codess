...
import {
  ...
  prettierLessOpt,
  prettierScssOpt,
  compExcludeClassnames,
  notRenderChildrenComps,
} from './consts';

export default function exportMod(schema, option): Promise<IPanelDisplay[]> {
  return new Promise(async (resolve) => {
    ...
    const isExportGlobalFile =
      dslConfig.globalCss && blocksCount == 1 && !blockInPage;
    ...
    if (schema.componentName == 'Page') {
      ...
      folderName =
        pagesCount == 0 &&
        blocksCount == 1 &&
        dslConfig.outputStyle !== OUTPUT_TYPE.PROJECT
          ? ''
    ...
    const cssFileName = `${filePathName}${
      dslConfig.inlineStyle == CSS_TYPE.MODULE_CLASS ? '.module' : ''
    }.${dslConfig.cssType || 'css'}`;
    ...
    const generateRender = (json, isReplace = false): string => {
      ...
      switch (type) {
        ...
        case 'image':
          ...
          const exportName = addImgImport(
            folderName,
            imgFileMap[json.props.src]
          );
        ...
        case 'component':
          if (isReplace) {
            ...
            if (compName) {
              importMods.push(
                `import ${compName} from '${compPath}/${compName}';`
              );
            }
        ...
        default:
          if (componentName) {
            ...
            switch (componentData.name) {
              case 'Input':
                handleField(componentData, {
                  text: findText(json),
                });
            }

            Object.keys(componentData.props).forEach((key) => {
              const isVariable = Object.keys(
                componentData.state || {}
              ).includes(componentData.props[key]);
              props += ` ${key}={${parseProps(
                componentData.props[key],
                false,
                isVariable
              )}}`;
            });
            ...
            if (
              ...
              !notRenderChildrenComps.includes(componentName)
            ) {
      ...
      if (json.loop) {
        const parseLoopData = parseLoop(json.loop, json.loopArgs, xml, {});
    ...
    const handleHooksData = (json) => {
      ...
      if (json.state) {
        ...
        for (let ste of stateArr) {
          useState.push(parseState(ste, json.state[ste]));
        }
      }
    };
