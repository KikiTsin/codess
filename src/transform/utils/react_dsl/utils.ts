...
import tailwindMap from '../tailwindClassNames';
...
export const transComponentsMap = (compsMap: Record<string, any>) => {
...
export const tailwindStyle = (schema) => {
  traverseBrother(schema, function (nodes) {
    for (let node of nodes) {
      convertStyle2Tailwind(node, tailwindMap);
    }
  });
};
...
export const initSchema = (schema) => {
  ...
  traverse(schema, (json) => {
    ...
    switch (json.componentName.toLowerCase()) {
      case 'page':
        json.fileName = line2Hump(
          json.fileName || `page_${getCounter('page')}`
        );
        break;
      case 'block':
        json.fileName = line2Hump(
          json.fileName || `block_${getCounter('block')}`
        );
        break;
      case 'component':
        json.fileName = line2Hump(
          json.fileName || `component_${getCounter('component')}`
        );
        break;
      default:
        break;
...
export const parseProps = (value, isReactNode = false, isVariable = false) => {
...
export const generateCssString = (style) => {
  ...
  if (!style['margin'] && margin.length > 2) {
    style['margin'] = `${style['marginTop'] || 0} ${
      style['marginRight'] || 0
    } ${style['marginBottom'] || 0} ${style['marginLeft'] || 0}`;
  ...
  // 缩写 padding
  const padding = Object.keys(style).filter((item) =>
    item.startsWith('padding')
  );
  if (!style['padding'] && padding.length > 2) {
    style['padding'] = `${style['paddingTop'] || 0} ${
...
export const generateScss = (schema) => {
  ...
  function walk(json) {
    ...
    if (json.children && json.children.length > 0) {
      json.children.forEach((child) => {
        if (
          !['block', 'component', 'page'].includes(
            child.componentName.toLowerCase()
          )
        ) {
          walk(child);
        }
...
// parse state
export const parseState = (stateName, states) => {
  // hooks state
